...
1. `.ics` → parsed into `CalendarEvent[]`
2. Generate `TimeBucket[]` from user options
3. Bucket events by overlap with each bucket
4. For each Epic, sum **overlap hours** of matching events per bucket
5. Render a table: epics as rows, buckets as columns, color density by relative hours
...
- `src/ICSParser.ts`: converts ICS text → `CalendarEvent[]`
- `src/EpicTimelines.tsx`: top-level “app” logic once events are loaded (epics/options/buckets/selection)
- `src/BucketUtil.ts`: bucket generation, event bucketing, epic matching, and hours aggregation
...
- Tests:
  - `src/BucketUtil.test.ts`, `src/ICSParser.test.ts`, `src/Util.test.ts`
...
- `TimelineOptions`: `{ startDate, endDate, bucketGranularity, showBucketHours }`
...
- ICS repeating events are not expanded yet (`ICSParser.ts` has a TODO for repeating events / RRULE handling).
